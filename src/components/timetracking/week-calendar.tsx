...
export type Agrupacion = "cliente" | "proyecto" | "tarea";

function colorDe(registro: RegistroTiempoItem, agrupacion: Agrupacion) {
  if (agrupacion === "cliente") return registro.proyecto?.cliente?.color ?? "#64748b";
  return registro.proyecto?.color ?? "#64748b";
}

function etiquetaDe(registro: RegistroTiempoItem, agrupacion: Agrupacion) {
  if (agrupacion === "cliente") return registro.proyecto?.cliente?.nombre ?? "";
  if (agrupacion === "proyecto") return registro.proyecto?.nombre ?? "";
  return registro.tarea?.nombre ?? registro.proyecto?.nombre ?? "(sin tarea)";
}
...
export function WeekCalendar({
  ...
  return (
    <div className="flex overflow-x-auto rounded-lg border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
      <div className="w-14 shrink-0 border-r border-slate-100 dark:border-slate-800">
        <div className="h-10 border-b border-slate-100 dark:border-slate-800" />
      ...
      {dias.map((dia) => {
        ...
        return (
          ...
          >
            <div
              className={`flex h-10 flex-col items-center justify-center border-b border-slate-100 text-xs dark:border-slate-800 ${isToday ? "bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900" : "text-slate-600 dark:text-slate-300"}`}
            >
              ...
              {items.map(({ registro, left, width }) => (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: `${colorDe(registro, agrupacion)}33`,
                    borderLeft: `3px solid ${colorDe(registro, agrupacion)}`,
                  }}
                ...
                >
                  <div className="truncate font-medium">
                    {etiquetaDe(registro, agrupacion)}
                  </div>
